...
function inc(len) {
  if (len > 3) {
    let p;
    for (p = Math.ceil(len / 2); gcd(len, p) > 1; p++)
      ;
    return p;
  } else
    return 1;
...
class Sequence extends EventEmitter {
  ...
  /**
   * Change the length of the sequence. This will cause the increment to be recalculated, and may cause the
   * initial value to be adjusted.
  ...
  set length(n) {
    ...
    // adjust the initial value if necessary
    if (this.$len > 0)
      this.$init %= this.$len;
    else
      this.$init = 0;
